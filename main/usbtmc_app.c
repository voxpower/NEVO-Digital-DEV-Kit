...
static const char *TAG = "USBTMC APP";

uint8_t data_tx[SCPI_OUTPUT_BUFFER_LENGTH];  
...
bool tud_usbtmc_msg_data_cb(void *data, size_t len, bool transfer_complete)
{
  ...
  idnQuery = 0;
  //the scpi parser gets data that come from usbtmc 
  if (SCPI_Input(&scpi_usbtmc_context, data, len))
    idnQuery = 1;
  tud_usbtmc_start_bus_read();
  return true;
}
...
void usbtmc_app_task_iter(void) {
  ...
  case 4: // time to transmit;
    ...
    {
    	//scpi parser answers comes to usbtmc 
		uint32_t tam_tx = uxQueueMessagesWaiting(usbtmcTxQueue);
		uint32_t index = 0;
		uint8_t car;
		if (tam_tx)
		{
			index = 0;
			while (xQueueReceive(usbtmcTxQueue, &car, 1) != errQUEUE_EMPTY)
				data_tx[index++] = car;
			if (index)
			{
#ifdef DEBUG_SCPI						
				ESP_LOG_BUFFER_HEXDUMP(TAG, data_tx, index, ESP_LOG_INFO);
#endif				
			}
		}
        if(idnQuery && index)
      	{ 
      		tud_usbtmc_transmit_dev_msg_data(data_tx,  index,true,false);
      		queryState = 0;
      		bulkInStarted = 0;
      	}
        else
